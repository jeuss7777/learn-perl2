...
#
# With Perl 5
# This script has to be executed in the directory where the perl script is, besides this script
# there will be a folder called config which contain all the configuration files. The script and directory 
# should be place in Project/target/perl
#
# perl $MyScript $PathEntityClasses $OptionalWarName
#
# Example:
# perl Pr06_ScriptPerlGenerateLayerClasses.pl /home/jarana/workspace/Molly/entities MyProject
#
# perl Pr06_ScriptPerlGenerateLayerClasses.pl /home/jarana/workspace/Molly/entities 
#
#######################################################################################################
...
sub main {
	...
	`sed -i 's/MyPaCkAgE/'$PackBase'/g' configSpring/Initializer.java`;
	`sed -i 's/MyPaCkAgE/'$PackBase'/g' configSpring/WebAppConfig.java`;
	
	# cd ../src/main/java/com/learning/config/
	`cp configSpring/Initializer.java '$dirBase'/config/`;
	`cp configSpring/WebAppConfig.java '$dirBase'/config/`;

	print"Before Copying POM  ----------->    Directory:".getcwd."\n";
	`cp configSpring/pom.xml ../../`;

	# update pom.xml and copy
	`sed -i 's/GrOuPID/'$PackBase'/g' ../../pom.xml`;
	`sed -i 's/PrOjEcTNAME/'$projectDir'/g' ../../pom.xml`;
	`sed -i 's/artifactId>target/artifactId>'$projectDir'/g' ../../pom.xml`;
	`sed -i 's/FiNaLNAME/'$warFile'/g' ../../pom.xml`;
	`sed -i 's/finalName>target/finalName>'$warFile'/g' ../../pom.xml`;
	
	
	# if directory does not exists, create it
	print "\nHELLO***\n";
	# chdir("../../src/main");
	print"Point 1  ----------->    Directory:".getcwd."\n";
	chdir('../../src/main');
	print"After Copying POM  ----------->    Directory:".getcwd."\n";
	if (!-d "resources") {
		print "\nHELLO2***";
	...
	# myActualDir:/home/jarana/eclipse-workspace/SpringSecurity/src/main/java/com/jarana/entities
	# startDir:/home/jarana/eclipse-workspace/SpringSecurity/target/perl

	
	# applications.properties
	`cp $startDir/configSpring/application.properties ../../src/main/resources/`;
	`sed -i 's/MyPaCkAgE/'$PackBase'/g' ../src/main/resources/application.properties`;
	
	# Copying logback file
	`cp $startDir/configSpring/logback.xml ../../src/main/resources/`;
	
	chdir("../../src/main/resources/");
	my $propertBase = getcwd;

	print "\n*** NOTE.- Don't forget to set up JDBC DB Connection settings at\n";
	print "$propertBase/application.properties\n\n";
	chdir($startDir);
	system('find ~/.m2  -name "*.lastUpdated" -exec grep -q "Could not transfer" {} \; -print -exec rm {} \;');
